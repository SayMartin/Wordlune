...
-- 2) upsert categories (unchanged: do NOT modify name_en/name_sv)
...
-- 3) upsert subcategories (unchanged: do NOT modify name_en/name_sv)
WITH cats AS (
...
-- ONLY word_en and word_sv are uppercased on insert
WITH existing AS (
  SELECT
    w.id,
    w.word_native,
...
-- only uppercasing word_en/word_sv when updating; native fields remain canonical
WITH existing AS (
  SELECT
    w.id,
    w.word_native,
...
-- 5) link words -> categories (CASE-INSENSITIVE joins)
WITH w AS (
...
-- 6) link words -> subcategories (CASE-INSENSITIVE joins)
WITH w AS (
  SELECT id, word_native, word_en FROM public.words
), s AS (
