...
- **`20260826_challenge_rotation_reset.sql`** — **destructive, and not yet applied.** Wipes every challenge, `challenge_attempt`, `challenge_result` and challenge-linked `game_scores` row, then rebuilds the feature: adds `end_date`, rebuilds `challenge_menu_stats` to show only the active window and to resolve category names from `subcategories` at read time in all three languages, and replaces the table's `SELECT USING (true)` with a `start_date <= now()` gate. Three things forced it together: the January challenges named subcategories retired in `20260825` (because `description` froze the names at generation time); `20260131_add_is_five_restricted.sql` added `is_five_chars` to the table but never to the view, so the 5x5 flag has never once reached the client; and the table has been world-readable since `20260127` — the GDPR lockdown never covered it — so publishing a schedule in advance would publish the answers with it. The wipe is also a scoring reset: a word is now worth up to 150 points instead of 100 (`src/utils/scoring.ts`), and the two scales cannot share a leaderboard. Rehearsed against a local `supabase start` stack, including verifying from outside with the anon key that 0 of 35 queued challenges are visible. Run it, then run `generate_challenge.mjs`.

## scripts/

Run from this directory (`supabase-ops/`) so the `migrations/...` relative paths inside them resolve correctly:

- **`generate_challenge.mjs`** — lays down the competitive challenge schedule: 26 weekly themed challenges plus a 5x5 every third week, ~six months ahead, each with a `start_date`/`end_date` window so only the current week is offered. Reads Supabase credentials from a local `.env` file in this directory, or from the environment. It accepts whichever spelling is there — `SUPABASE_URL` / `VITE_SUPABASE_URL` / `EXPO_PUBLIC_SUPABASE_URL` for the project URL, and `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_SERVICE_KEY` for the key — because the scripts in this directory historically disagreed about the names and "missing credentials" against a `.env` that plainly has them is undiagnosable without opening a file nobody should open. When it still can't find them it prints the key *names* defined in the file (never values) alongside the ones it accepts. **Run `--dry-run` first** — it prints the whole schedule with the chosen words and writes nothing. Requires `20260826_challenge_rotation_reset.sql` to have been applied, and the service-role key specifically: unstarted challenges are invisible to anon and authenticated by design, so a lesser key can neither insert them nor read them back.

  It aborts rather than degrades, which is the point. The previous version hardcoded subcategory slugs, printed a warning when one didn't resolve, and carried on with a smaller set — which is how challenges came to advertise retired categories. Now an unknown or answer-ineligible subcategory name is a hard failure, and so is a word pool too thin to fill the batch with 1.4× headroom left over. Every word is checked against `Keyboard.tsx`'s `LAYOUTS` character sets (not `LETTER_PATTERNS` — see the note further down), must exist in all three languages, and is drawn without replacement across the whole batch.

  Brand lists are deliberately excluded from challenges even though they are answer-eligible: they are the same content `20260825_remove_bicycle_brands_and_at_sea.sql` retired Bicycle Brands for — proper nouns spelled identically in all three languages, so neither the category hint nor the language tells the player anything. They stay fully playable in practice mode.

  `scripts/migration_state.sql` reports the remaining runway ("N live, M queued, last starts …"). When `queued` hits zero the challenge menu empties and competitive mode becomes unplayable, so re-run this well before then.

- **`20260829_neutral_challenge_names.sql`** — **not yet applied.** Non-destructive; rewrites a display string only. The first generated batch was named after its theme ("Africa — week of 2026-09-21"), and the name is the one thing a player sees before starting — which is when a leaderboard-counting clock begins, now that time is part of the score. Renames them to `Weekly Challenge — <date>` / `5x5 Challenge — <date>`; "5x5" stays because it is a rule rather than content. Scoped to the generator's own name pattern so a hand-named challenge is untouched, and idempotent. The generator produces the neutral form directly from now on.
- **`20260828_duel_timeouts.sql`** — **not yet applied.** Non-destructive. Duels had no time limit at all: `duelElapsed` counts upward for display only, and presence sync only catches a *clean* disconnect, so a player who walked away with the tab open left their opponent waiting forever and the row at `status='playing'`. Adds the ten columns the server needs to judge (per-player guess count, best-guess greens/yellows, activity timestamps), `record_duel_progress()` for clients to report after each guess, `resolve_duel()` as the single row-locked judge, and `sweep_stale_duels()` on a 5-minute `pg_cron` job for matches both players abandoned. Also re-creates `join_duel_match()` verbatim apart from stamping `started_at`. Rules: 2 minutes of one-sided inactivity forfeits; 8 minutes of mutual silence is decided on proximity; never a draw. Rehearsed locally against six cases — one-sided idle, both idle under the cap, both idle over it, a greens tie falling through to guess count, a player who has used all six guesses being ineligible either way, and idempotency under a double call.
- **`20260827_replay_and_duel_history.sql`** — **not yet applied.** Non-destructive. Four things the rebuilt Progress screen needs: (1) `challenge_results` still carried `FOR SELECT TO authenticated USING (true)` alongside its own-row policy — verifiably in the 20260825 baseline, so the August lockdown missed it and every player's challenge scores have been readable by any signed-in account, guests included; (2) `restart_challenge_attempt()` makes a finished challenge replayable while its week is open, resetting the attempt row rather than breaking `UNIQUE(player_id, challenge_id)`; (3) `duel_matches` gains `player1_score`/`player2_score`/`finished_at`, which never existed — duel scores lived only in React state, so history could show who won but never by how much (older duels stay NULL and the UI shows a dash); (4) `duel_leaderboard` unions both sides of every finished duel so losses exist at all, exposes `diff` (wins − losses) and is ranked on it rather than on wins — ranking wins alone ranks whoever has played most, putting a 20–15 record above a 9–0 one — and `my_duel_history` gives a player their own duels with the opponent's name. `leaderboard_entries` now ranks each player's **first** completed run of a challenge, not their best — on a replay the five words are already known, so ranking the best would rank willingness to replay. Rehearsed locally against the `authenticated` role with real JWT claims: cross-player reads return 0 rows, restart zeroes an open attempt, and restart on a closed challenge raises.
